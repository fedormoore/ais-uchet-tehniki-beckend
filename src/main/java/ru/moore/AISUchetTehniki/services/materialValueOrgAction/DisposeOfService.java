...
public class DisposeOfService {
    ...
    public List<MaterialValueOrgDto> saveDisposeOf(List<DisposeOfDto> disposeOfDtoList) {
        try {
            ...
            for (int i = 0; i < disposeOfDtoList.size(); i++) {
                for (int j = 0; j < disposeOfDtoList.get(i).getSpecification().size(); j++) {
                    addDeviceSpecificationDtoList.add(disposeOfDtoList.get(i).getSpecification().get(j));
                }
            }
