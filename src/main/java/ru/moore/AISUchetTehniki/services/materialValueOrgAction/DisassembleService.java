...
public class DisassembleService {
    ...
    public List<MaterialValueOrgDto> saveDisassemble(List<DisassembleDto> disassembleDtoList) {
        try {
            ...
            for (int i = 0; i < disassembleDtoList.size(); i++) {
                for (int j = 0; j < disassembleDtoList.get(i).getSpecification().size(); j++) {
                    disassembleSpecDtoList.add(disassembleDtoList.get(i).getSpecification().get(j));
                }
            }
